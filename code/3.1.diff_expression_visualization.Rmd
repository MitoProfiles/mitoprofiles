...
combined_de_with_interaction_plots$all <- plot_extreme_degs(diff_exprs_list = combined_de_with_interaction,
                                            n_degs_to_plot = 15,
                                            mito_yes_no = "no")
combined_de_no_interaction_plots$all <- plot_extreme_degs(diff_exprs_list = combined_de_no_interaction,
                                               n_degs_to_plot = 15,
                                               mito_yes_no = "no")
combined_de_no_interaction_plots <- plot_extreme_degs(diff_exprs_list = combined_de_only_interaction,
                                               n_degs_to_plot = 15,
                                               mito_yes_no = "no")
...
combined_de_with_interaction_plots$mito <- plot_extreme_degs(diff_exprs_list = combined_de_with_interaction,
                                            n_degs_to_plot = 15,
                                            mito_yes_no = "yes")
combined_de_no_interaction_plots$mito <- plot_extreme_degs(diff_exprs_list = combined_de_no_interaction,
                                               n_degs_to_plot = 15,
                                               mito_yes_no = "yes")
combined_de_no_interaction_plots$mito <- plot_extreme_degs(diff_exprs_list = combined_de_only_interaction,
                                               n_degs_to_plot = 15,
                                               mito_yes_no = "no")
...
## Visualize Coefficients, LFC, and CPMs of the different datasets and linear models
...
lrt_df$coef_gtex <- data.frame(gene_id = rownames(gtex_de$lrt$coefficients), gtex_de$lrt$coefficients, row.names = NULL)
colnames (lrt_df$coef_gtex)[-1] <- paste0(colnames(lrt_df$coef_gtex)[-1], "_gtex")

lrt_df$coef_tcga <- data.frame(gene_id = rownames(tcga_de$lrt$coefficients), tcga_de$lrt$coefficients, row.names = NULL)
colnames (lrt_df$coef_tcga)[-1] <- paste0(colnames(lrt_df$coef_tcga)[-1], "_tcga")

lrt_df$coef_combined_no_interaction <- data.frame(gene_id = rownames(combined_de_no_interaction$lrt$coefficients), combined_de_no_interaction$lrt$coefficients, row.names = NULL)
colnames (lrt_df$coef_combined_no_interaction)[-1] <- paste0(colnames(lrt_df$coef_combined_no_interaction)[-1], "_comb_NO")

lrt_df$coef_combined_only_interaction <- data.frame(gene_id = rownames(combined_de_only_interaction$lrt$coefficients), combined_de_only_interaction$lrt$coefficients, row.names = NULL)
colnames (lrt_df$coef_combined_only_interaction)[-1] <- paste0(colnames(lrt_df$coef_combined_only_interaction)[-1], "_comb_ONLY")

lrt_df$coef_combined_with_interaction <- data.frame(gene_id = rownames(combined_de_with_interaction$lrt$coefficients), combined_de_with_interaction$lrt$coefficients, row.names = NULL)
colnames (lrt_df$coef_combined_with_interaction)[-1] <- paste0(colnames(lrt_df$coef_combined_with_interaction)[-1], "_comb_WITH")


# Merge all dataframes for common gene ids
lrt_df$coef_merged <- Reduce(function(x,y) inner_join (x,y, by="gene_id"), lrt_df)


# Pivot the dataframe to allow drawing boxplots
lrt_df$coef_merged_long <- lrt_df$coef_merged %>%
  pivot_longer(cols = !gene_id,
               cols_vary = "slowest",
               names_to = "coeff_name",
               values_to = "coeff_value") %>%
  mutate(coeff_name = factor(coeff_name,
                                levels = colnames(lrt_df$coef_merged)[-1]))


# Boxplots | View the distributions of all coefficients (using only common genes)
lrt_df$coef_merged_long %>%
...
# Visualize Log Fold Change values
par(mfrow=c(3,2))

hist(gtex_de$topGenes$table$logFC,
     main = "GTEx | (~ 0 + metabolic_rate) Log FC",
     xlab = "Log Fold Change")
hist(tcga_de$topGenes$table$logFC,
     main = "TCGA | (~ 0 + metabolic_rate) Log FC",
     xlab = "Log Fold Change")
hist(
  combined_de_no_interaction$topGenes$table$logFC,
  main = "Combined NO interaction | (~ 0 + MR + CS) Log FC",
  xlab = "Log Fold Change")
hist(
  combined_de_only_interaction$topGenes$table$logFC,
  main = "Combined ONLY interaction | (~ 0 + MR : CS) Log FC",
  xlab = "Log Fold Change")
hist(
  combined_de_with_interaction$topGenes$table$logFC,
  main = "Combined WITH interaction | (~ 0 + MR + CS + MR : CS) Log FC",
  xlab = "Log Fold Change")


# Visualize Log Counts per Million
par(mfrow=c(3,2))

hist(gtex_de$topGenes$table$logCPM,
     main = "GTEx | (~ 0 + metabolic_rate) Log CPM",
     xlab = "Log Counts per Million")
hist(tcga_de$topGenes$table$logCPM,
     main = "TCGA | (~ 0 + metabolic_rate) Log CPM",
     xlab = "Log Counts per Million")
hist(
  combined_de_no_interaction$topGenes$table$logCPM,
  main = "Combined NO interaction | (~ 0 + MR + CS) Log CPM",
  xlab = "Log Counts per Million")
hist(
  combined_de_only_interaction$topGenes$table$logCPM,
  main = "Combined ONLY interaction | (~ 0 + MR : CS) Log CPM",
  xlab = "Log Counts per Million")
hist(
  combined_de_with_interaction$topGenes$table$logCPM,
  main = "Combined WITH interaction | (~ 0 + MR + CS + MR : CS) Log CPM",
  xlab = "Log Counts per Million")
