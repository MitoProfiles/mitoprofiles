...
# Load data
gtex <- readRDS(here("data/analysis/gtex_de.RDS"))
tcga <- readRDS(here("data/analysis/tcga_de.RDS"))
combined_tcga_gtex <- readRDS(here("data/analysis/combined_de_with_interaction.RDS"))
...
# GO analysis
gtex_fun$go <- edgeR::goana.DGELRT(gtex$lrt, 
...
# KEGG analysis
gtex_fun$kegg <- edgeR::kegga.DGELRT(gtex$lrt, 
                                     geneid = gtex_fun$entrez_ids,
                                     species = "Hs")
...
# GO analysis
tcga_fun$go <- edgeR::goana.DGELRT(tcga$lrt, 
...
# KEGG analysis
tcga_fun$kegg <- edgeR::kegga.DGELRT(tcga$lrt, 
                                     geneid = tcga_fun$entrez_ids,
                                     species = "Hs")
...
# Get Entrez Gene identifiers (required by edgeR functions)
rownames(combined_tcga_gtex$lrt) %>%
...
# GO analysis
combined_fun$go <- edgeR::goana.DGELRT(combined_tcga_gtex$lrt, 
...
# KEGG analysis
combined_fun$kegg <- edgeR::kegga.DGELRT(combined_tcga_gtex$lrt, 
                                     geneid = combined_fun$entrez_ids,
                                     species = "Hs")
...
gtex_fun$gprofiler <- gprofiler2::gost(
  query = gsub('\\.\\d+', '', rownames(gtex$topGenes$table)[1:500]),
  organism = "hsapiens",
  ordered_query = FALSE,
  user_threshold = 0.05,
...
tcga_fun$gprofiler <- gprofiler2::gost(
  query = gsub('\\.\\d+', '', rownames(tcga$topGenes$table)[1:500]),
  organism = "hsapiens",
  ordered_query = FALSE,
  user_threshold = 0.05,
  correction_method = "fdr",
...
combined_fun$gprofiler <- gprofiler2::gost(
  query = gsub('\\.\\d+', '', rownames(combined_tcga_gtex$topGenes$table)[1:500]),
  organism = "hsapiens",
  ordered_query = FALSE,
  user_threshold = 0.05,
  correction_method = "fdr",
