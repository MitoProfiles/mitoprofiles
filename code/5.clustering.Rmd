...
pdf((file=here("analysis_output/heatmaps/heatmap_with_interaction.pdf")), 
    width = 10, height = 8)  

  heatmap(dgelist_log2cpm_mito_ordered, 
  ...
  # Column color labels
  mtext( "Bladder", side=3, cex = 1, line=2.65, at=c(0.175))
  mtext( "NC", side=3, cex = 0.7, line=2, at=c(0.15))
  mtext( "C", side=3, cex = 0.7, line=2, at=c(0.18))
  
  mtext( "Skin", side=3, cex = 1, line=2.62, at=c(0.3))
  mtext( "NC", side=3, cex = 0.7, line=2, at=c(0.25))
  mtext( "C", side=3, cex = 0.7, line=2, at=c(0.34))
  
  mtext( "Colon", side=3, cex = 1, line=2.62, at=c(0.53))
  mtext( "NC", side=3, cex = 0.7, line=2, at=c(0.45))
  mtext( "C", side=3, cex = 0.7, line=2, at=c(0.565))
  
  mtext( "Brain", side=3, cex = 1, line=2.62, at=c(0.63))
  mtext( "NC", side=3, cex = 0.7, line=2, at=c(0.61))
  mtext( "C", side=3, cex = 0.7, line=2, at=c(0.67))
  
  mtext( "Liver", side=3, cex = 1, line=2.62, at=c(0.74))
  mtext( "NC", side=3, cex = 0.7, line=2, at=c(0.72))
  mtext( "C", side=3, cex = 0.7, line=2, at=c(0.79))
  
  mtext( "Kidney", side=3, cex = 1, line=2.62, at=c(0.9))
  mtext( "NC", side=3, cex = 0.7, line=2, at=c(0.865))
  mtext( "C", side=3, cex = 0.7, line=2, at=c(0.91))
...
```{r fuzzy_clustering, eval=FALSE}

set.seed(3)

## Run cmeans clustering algorithm, with 20 clusters, and 1.25 fuzzification parameter m
cmeans_res <- e1071::cmeans(dgelist_log2cpm_mito_ordered, centers=20, m=1.25, dist = "manhattan", method = "cmeans")
...
my_mfuzz.plot2(dgelist_log2cpm_mito_ordered, cl = cmeans_res, mfrow = c(2, 2),
               ...
               centre = TRUE, centre.col = "gray10", centre.lwd = 0.1)
...
output_dir <- here("analysis_output/fuzzy_clustering")
dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)


# Create a loop to generate and save plots for each set of 4 clusters
num_clusters <- ncol(cmeans_res$membership)
clusters_per_page <- 4

for (page in 1:(num_clusters / clusters_per_page)) {
  start_cluster <- (page - 1) * clusters_per_page + 1
  end_cluster <- start_cluster + clusters_per_page - 1

  # Create a sub-cluster object for the current clusters
  sub_cmeans_res <- cmeans_res
  sub_cmeans_res$membership[, -(start_cluster:end_cluster)] <- 0
  
  # Generate the plot
  pdf_file <- here(output_dir, paste0("fuzzy_clustering_page", page, ".pdf"))
  pdf(pdf_file, width = 8, height = 6)  
  my_mfuzz.plot2(dgelist_log2cpm_mito_ordered, cl = sub_cmeans_res, mfrow = c(2, 2),
                 x11 = FALSE, xlab = "Samples",
                 centre = TRUE, centre.col = "gray10", centre.lwd = 0.1)
  dev.off()
}
